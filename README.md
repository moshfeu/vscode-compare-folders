...
- `ignoreList` - string[] - list of strings to ignore when comparing file contents. Files that differ only by these strings are treated as identical. Example: `["dev", "staging", "prod"]` to compare configuration files across environments. Only applies when `compareContent` is enabled. Binary files are always compared normally. ⚠️ This option reads and transforms every compared text file in memory and may be slow on large files.
